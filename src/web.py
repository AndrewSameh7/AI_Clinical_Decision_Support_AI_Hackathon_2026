...
app=FastAPI(title="AI Clinical Decision Support Lite",version="3.0.0")
...
USER_STORAGE_LIMIT_BYTES = int(os.getenv("USER_STORAGE_LIMIT_BYTES","2097152"))
...
def db():
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS daily_usage(user_id TEXT NOT NULL,usage_date TEXT NOT NULL,question_count INTEGER NOT NULL DEFAULT 0,PRIMARY KEY(user_id,usage_date),FOREIGN KEY(user_id) REFERENCES users(id))""")
    c.commit(); return c
...
def _daily_payload(c,user_id):
    row=c.execute("SELECT question_count FROM daily_usage WHERE user_id=? AND usage_date=?",(user_id,_today())).fetchone()
    used=int(row["question_count"]) if row else 0
    return {"used":used,"limit":DAILY_QUESTION_LIMIT,"remaining":max(0,DAILY_QUESTION_LIMIT-used),"exhausted":used>=DAILY_QUESTION_LIMIT,"date":_today()}

def _reserve_daily_question(c,user_id):
    today=_today()
    c.execute("INSERT OR IGNORE INTO daily_usage(user_id,usage_date,question_count) VALUES(?,?,0)",(user_id,today))
    cur=c.execute("UPDATE daily_usage SET question_count=question_count+1 WHERE user_id=? AND usage_date=? AND question_count<?",(user_id,today,DAILY_QUESTION_LIMIT))
    if cur.rowcount != 1:
        c.rollback()
        return False
    c.commit()
...
def me(request:Request):
    u=session_user(request); c=db(); storage=_storage_payload(c,u["id"]); daily=_daily_payload(c,u["id"]); c.close()
    return {"user":{**u,"role":USER_ROLE},"storage":storage,"daily_usage":daily}
...
def retrieve_evidence(x:RetrieveRequest,request:Request):
    ...
    c=db()
    if not _reserve_daily_question(c,u["id"]):
        daily=_daily_payload(c,u["id"]); c.close()
        raise HTTPException(429, f"Daily question limit reached. You have used all {DAILY_QUESTION_LIMIT} questions for today. Your limit resets tomorrow.")
    daily=_daily_payload(c,u["id"]); c.close()
    ...
    if blocked:
        ...
        return {"retrieval_id":rid,"question":q,"scope":CLINICAL_SCOPE,"strategy":x.strategy,"config":x.config,"top_k":1,"evidence":[],"pre_result":blocked,"daily_usage":daily}
    if _needs_clarification(q):
        ...
        return {"retrieval_id":rid,"question":q,"scope":CLINICAL_SCOPE,"strategy":x.strategy,"config":x.config,"top_k":1,"evidence":[],"pre_result":pre,"daily_usage":daily}
    ...
    return {"retrieval_id":rid,"question":q,"scope":CLINICAL_SCOPE,"strategy":x.strategy,"config":x.config,"top_k":1,"evidence":[_public_evidence(rows[0])] if rows else [],"daily_usage":daily}
